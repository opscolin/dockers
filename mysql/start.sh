...
{
	StartMySQL
	if [ "$MYSQL_PASSWD" = "**RANDOM**" ]; then
	    unset MYSQL_PASSWD
	fi

	#PASS=${MYSQL_PASS:-$(pwgen -s 12 1)}
	PASS=${MYSQL_PASSWD:-$(date +%s | sha256sum | base64 | head -c 12 ; echo)}
	_word=$( [ ${MYSQL_PASSWD} ] && echo "preset" || echo "random" )
	...
	echo "MySQL user 'root' has no password but only allows local connections except you set MYSQL_USER='root'"
	echo "========================================================================"
